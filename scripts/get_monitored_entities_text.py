#!/usr/bin/env python3
"""
v7.1-F.2 — Reads monitored_entities.json and returns formatted list
...
Usage:
  ...
  get_monitored_entities_text.py light climate            # domain subset
...
# Entities whose entity_id is not descriptive for a prompt (names = id)
...
def main():
    ...
    if not monitored:
        print("(no entities monitored at this time)")
        return
    ...
    for e in monitored:
        ...
        # Format: "entity_id (Friendly Name)" or just "entity_id" if name obvious
    ...
    if not by_domain:
        print("(no entities in requested domain(s))")
        return
