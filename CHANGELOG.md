...
- `event_log` table (insert-only, 30-day retention): records every event occurrence
  with timestamp and per-type metadata, independent of the daily `event_buffer`.
  This is the history source for temporal correlation (planned for v8.6). Nothing
  reads it yet — it only accumulates.
...
- **Timezone mismatch in memory queries.** Event timestamps are stored in local
  time, but several "today"/window queries filtered using SQLite UTC (`date('now')`).
  At night this made the day's events invisible to Sleep Consolidation, which then
  persisted nothing. All such queries now use local time consistently.
- `systems_compile.py` argument mismatch (retired `soul`/`users` parameters removed;
  the script now takes a single `insights` argument).
- `ha_update_manager.py` no longer errors on an empty or unrendered `entity_id`.

### Added

- **Enriched event capture.** `event_buffer` now stores a small, fixed set of
  per-type attributes in a `metadata` JSON field — media title, cover position,
  light brightness/duration, climate temperature/setpoint — propagated to memory.
  Capture is deliberately selective, not "everything Home Assistant exposes."

### Changed

- Cover state triggers debounced (3s) so a flapping device cannot fabricate a
  false pattern.
